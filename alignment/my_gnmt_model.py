...
class MyGNMTModel(my_attention_model.MyAttentionModel):
    ...
    def __init__(self,
        ...
        super(MyGNMTModel, self).__init__(
            hparams=hparams,
            mode=mode,
            iterator=iterator,
            scope=scope,
    ...
    def _build_encoder(self, hparams):
        ...
        if hparams.encoder_type == "uni" or hparams.encoder_type == "bi":
            return super(MyGNMTModel, self)._build_encoder(hparams)
    ...
    def _build_decoder_cell(self, hparams, encoder_outputs, encoder_state,
        ...
        if not self.is_gnmt_attention:
            return super(MyGNMTModel, self)._build_decoder_cell(
    ...
    def get_alignment_history(self):
        return self.final_context_state[0].alignment_history.stack()

    def _get_infer_summary(self, hparams):
        if hparams.infer_mode == "beam_search":
            return tf.no_op()
        elif self.is_gnmt_attention:
            return my_attention_model._create_attention_images_summary(
                self.final_context_state[0])
        else:
            return super(MyGNMTModel, self)._get_infer_summary(hparams)


class MyGNMTAttentionMultiCell(tf.nn.rnn_cell.MultiRNNCell):
    ...
    def __init__(self, attention_cell, cells, use_new_attention=False):
        ...
        super(MyGNMTAttentionMultiCell, self).__init__(cells, state_is_tuple=True)
