...
class BaseModel(object):
    ...
    def __init__(self,
        ...
        if holders:
            ...
            self.source_sequence_length = tf.constant(np.array([15]), dtype=tf.int32)
            self.target_sequence_length = tf.constant(np.array([10]), dtype=tf.int32)
        # Set params
    ...
    def _build_decoder(self, encoder_outputs, encoder_state, hparams):
        ...
        with tf.variable_scope("decoder") as decoder_scope:
            ...
            ## Train or eval or inference
            # decoder_emp_inp: [max_time, batch_size, num_units]
            # target_input = iterator.target_input
            # if self.time_major:
            #     target_input = tf.transpose(target_input)
            decoder_emb_inp = tf.nn.embedding_lookup(
                self.embedding_decoder, self.target_input)
    ...
    def decode(self, sess, feed):
        ...
        """
        output_tuple = self.infer(sess, feed)
...
class Model(BaseModel):
    ...
    def _build_encoder(self, hparams):
        ...
        return self._build_encoder_from_sequence(
            hparams, self.source, self.source_sequence_length)
