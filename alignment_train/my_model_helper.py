...
def create_serving_train_model(
    ...
    with graph.as_default(), tf.container(scope or "train"):
        ...
        src_dataset = tf.data.TFRecordDataset(src_file)
        tgt_dataset = tf.data.TFRecordDataset(tgt_file)
        skip_count_placeholder = tf.placeholder(shape=(), dtype=tf.int64)

        iterator = my_iterator_utils.get_serving_iterator(
            src_dataset,
            tgt_dataset,
            #src_vocab_table,
            #tgt_vocab_table,
...
def create_serving_eval_model(model_creator, hparams, scope=None, extra_args=None):
    """Create train graph, model, src/tgt file holders, and iterator."""
    #src_vocab_file = hparams.src_vocab_file
    #tgt_vocab_file = hparams.tgt_vocab_file
    graph = tf.Graph()
