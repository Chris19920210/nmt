...
def run_internal_eval(eval_model,
  ...
  with eval_model.graph.as_default():
    loaded_eval_model, global_step = my_model_helper.create_or_load_model(
...
def run_external_eval(infer_model,
  ...
  with infer_model.graph.as_default():
    loaded_infer_model, global_step = my_model_helper.create_or_load_model(
        infer_model.model, model_dir, infer_sess, "infer")
  ...
  dev_infer_iterator_feed_dict[
      infer_model.tgt_placeholder] = my_inference.load_data(dev_src_file, dev_tgt_file)
  dev_infer_iterator_feed_dict[
  ...
  if use_test_set and hparams.test_prefix:
    ...
    test_infer_iterator_feed_dict[
        infer_model.src_placeholder],
    test_infer_iterator_feed_dict[
        infer_model.tgt_placeholder] = my_inference.load_data(test_src_file, test_tgt_file)
    test_infer_iterator_feed_dict[
        infer_model.batch_size_placeholder] = hparams.infer_batch_size
...
def run_avg_external_eval(infer_model, infer_sess, model_dir, hparams,
  ...
  if hparams.avg_ckpts:
    ...
    avg_model_dir = my_model_helper.avg_checkpoints(
...
def train(hparams, scope=None, target_session=""):
  ...
  train_model = my_model_helper.create_serving_train_model(model_creator, hparams, scope)
  eval_model = my_model_helper.create_serving_eval_model(model_creator, hparams, scope)
  infer_model = my_model_helper.create_serving_infer_model(model_creator, hparams, scope)

  # Preload data for sample decoding.
  dev_src_file = "%s.%s" % (hparams.dev_prefix, hparams.src)
  dev_tgt_file = "%s.%s" % (hparams.dev_prefix, hparams.tgt)
  sample_src_data, sample_tgt_data = inference.load_data(dev_src_file, dev_tgt_file)
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
                   tgt_data, iterator_src_placeholder, iterator_tgt_placeholder,
