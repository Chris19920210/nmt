...
def convert_graph_def_to_saved_model(export_dir, graph_filepath):
    ...
    with tf.Session(graph=tf.Graph()) as session:
        ...
        tf.saved_model.simple_save(
            ...
            export_dir,
            inputs={
                'input': session.graph.get_tensor_by_name('src_placeholder:0')},
            outputs={'outputs': session.graph.get_tensor_by_name('translation:0'),
                     'scores': session.graph.get_tensor_by_name('beam_score:0')}
        )
...
if __name__ == "__main__":
    ...
    if args.saved_model is not None:
        freeze_model(args.saved_model, "translation,beam_score", os.path.join(args.export_dir, "frozen_model.pb"))

    optimize_graph(args.export_dir, "frozen_model.pb" if args.freeze_model is None else args.freeze_model,
                   args.transforms, ["translation", "beam_score"])
